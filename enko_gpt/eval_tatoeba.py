...
def retrieve(pooler, batch_size):
    data_dir = 'path/to/mSimCSE/data/Tatoeba/v1/'
    src_lang_dict = {'kor':'ko'}
    lang_dict = {l2: l3 for l3, l2 in src_lang_dict.items()}
    lg_list = 'ko'.split()
    all_acc, all_acc_jsonl = [],[]
    
    for lg in lg_list:
        lg = lang_dict[lg]
        src_file = f'{data_dir}tatoeba.{lg}-eng.eng'
        src_sentences = [line.strip() for line in open(src_file)]
        tgt_file = f'{data_dir}tatoeba.{lg}-eng.{lg}'
