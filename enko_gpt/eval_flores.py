...
model = GPT2LMHeadModel.from_pretrained(model_path).eval().to("cuda:1")
...
def retrieve(pooler, batch_size):
    data_dir = "path/to/internship_dayeonki/dream_recons/flores200/dev/"
    all_acc, all_acc_jsonl = [],[]
    
    src_lang = "eng_Latn.dev"
    tgt_lang = "kor_Hang.dev"

    src_file = f'{data_dir}{src_lang}'
    src_sentences = [line.strip() for line in open(src_file)]
    tgt_file = f'{data_dir}{tgt_lang}'
    tgt_sentences = [line.strip() for line in open(tgt_file)]
    all_acc, all_acc_jsonl = [],[]
