...
def retrieve_bucc(args, model, pooler, device):
    ...
    for lg in ["fr","ru","zh","de"]:
        ...
        src_semantic_embed = semantic_embedding_laser(model, pooler, src_text, args.batch_size, lg, device).cpu().detach().numpy()
        src_language_embed = language_embedding_laser(model, pooler, src_text, args.batch_size, lg, device).cpu().detach().numpy()

        tgt_id, tgt_text, tgt_id2text = process_text(tgt_file)
        tgt_semantic_embed = semantic_embedding_laser(model, pooler, tgt_text, args.batch_size, "en", device).cpu().detach().numpy()
        tgt_language_embed = language_embedding_laser(model, pooler, tgt_text, args.batch_size, "en", device).cpu().detach().numpy()

        sem_candidate_file = data_dir + f"{lg}-{args.model_nickname}-candidate.sem.tsv"
        lang_candidate_file = data_dir + f"{lg}-{args.model_nickname}-candidate.lang.tsv"
        ...
        all_f1_semantic.append(semantic_result["f1"])
        all_f1_language.append(language_result["f1"])
...
def main():
    ...
    parser.add_argument("--model_nickname", type=str)
    ...
    pooler = PoolerClass(embedding_size=1024, n_languages=args.n_languages)
    pooler.load_state_dict(torch.load(args.pooler_path))
    pooler.to(device)

    retrieve_bucc(model, pooler, device)
