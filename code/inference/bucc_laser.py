...
def retrieve_bucc(args, model, pooler, device):
    ...
    for lg in ["fr","ru","zh","de"]:
        ...
        src_semantic_embed = semantic_embedding(model, pooler, src_text, args.batch_size, lg, device).cpu().detach().numpy()
        src_language_embed = language_embedding(model, pooler, src_text, args.batch_size, lg, device).cpu().detach().numpy()

        tgt_id, tgt_text, tgt_id2text = process_text(tgt_file)
        tgt_semantic_embed = semantic_embedding(model, pooler, tgt_text, args.batch_size, "en", device).cpu().detach().numpy()
        tgt_language_embed = language_embedding(model, pooler, tgt_text, args.batch_size, "en", device).cpu().detach().numpy()
...
def main():
    ...
    retrieve_bucc(args, model, pooler, device)
